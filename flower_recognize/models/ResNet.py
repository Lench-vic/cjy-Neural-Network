...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride, downsample=None):
        ...
        self.conv3 = nn.Conv2d(planes, planes * self.extention, kernel_size=(1, 1), stride=(1, 1), bias=False)
...
class ResNet(nn.Module):
    def __init__(self, block=Bottleneck, layers=None, num_class=1000):
        # inplane=当前的fm的通道数
        if layers is None:
            layers = [3, 4, 6, 3]
        self.inplane = 64
        ...
        # stem的网络层
        self.conv1 = nn.Conv2d(3, self.inplane, kernel_size=(7, 7), stride=(2, 2), padding=3, bias=False)
    ...
    def make_layer(self, block, plane, block_num, stride=1):
        ...
        if (stride != 1 or self.inplane != plane * block.extention):
            downsample = nn.Sequential(
                nn.Conv2d(self.inplane, plane * block.extention, stride=(stride, stride), kernel_size=(1, 1),
                          bias=False),
